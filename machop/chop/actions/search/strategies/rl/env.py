...
class MixedPrecisionEnv(gym.Env):
    ...
    def step(self, action):
        ...
        info = {"accuracy": software_metrics['accuracy'], "loss": software_metrics['loss'], "average_bitwidth": hardware_metrics['average_bitwidth']}
        print(info)
...
class MixedPrecisionEnvHiLo(gym.Env):
    ...
    def _action_to_config(self, action):
        ...
        for key, choices in self.search_space.choices_flattened.items():
            ...
            #if change != 0:
            #  print(f"Agent tried to change a value for key: {key}, Previous value: {current_value}, New Value: {new_value}")
